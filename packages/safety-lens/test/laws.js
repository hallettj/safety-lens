...
function lensLaws<S, A> (
  eq: (x: S, y: S) => boolean,
  { dataAndLens, value }: LensOpts<S, A>
) {
  return () => {
    property(
      'if you set something, you can get it back out',
      dataAndLens,
      value,
      ([obj, lens], val) => eq(get(lens, set(lens, val, obj)), val)
    )

    property(
      "getting and then setting doesn't change the answer",
      dataAndLens,
      ([obj, lens]) => eq(set(lens, get(lens, obj), obj), obj)
    )

    property(
      'putting twice is the same as putting once - the second put wins',
      dataAndLens,
      value,
      value,
      ([obj, lens], val1, val2) =>
...
function traversalLaws<S, A, FS: Apply<S>> (
  eq: (x: FS, y: FS) => boolean,
  { pure, dataAndTraversal }: TraversalOpts<S, A>
) {
  return () => {
